...
public class Video {
    ...
    @ElementCollection
    private Set<String> usersLikeIt = new HashSet();
    ...
    public synchronized boolean like(String user) {
        if (usersLikeIt.contains(user)) return false;
        usersLikeIt.add(user);
        likes = usersLikeIt.size();
        return true;
    }

    public synchronized boolean unLike(String user) {
        if (!usersLikeIt.contains(user)) return false;
        usersLikeIt.remove(user);
        likes = usersLikeIt.size();
        return true;
    }
